...
import {
  borcTipleri as seedRows,
  borcTipleriDetaylari as seedDetaylar,
} from '../../data/mockData'
...
export default function BorcTipleri() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => `${r.kod} ${r.ad} ${r.bsmv} ${r.tahakkuk}`.toLowerCase().includes(q))
  }, [rows, search])
  ...
  const openInspect = (row) => {
    setInfoModal({
      open: true,
      title: 'Borç Tipi İncele',
      body: (
        <div className="space-y-1 text-sm">
          <p><strong>Kod:</strong> {row.kod}</p>
          <p><strong>Ad:</strong> {row.ad}</p>
          <p><strong>BSMV:</strong> {row.bsmv}</p>
          <p><strong>Tahakkuk:</strong> {row.tahakkuk}</p>
        </div>
      ),
    })
    setMenuId(null)
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden">
      <ScreenHeader
        title="Borç Tipleri"
        description="Borç tipi tanımları ve operasyonel alt parametre detayları"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 flex-1 min-h-0">
        <div className="overflow-auto border-r border-slate-200">
          <table className="w-full grid-table">
            <thead>
              <tr>
                <th>Kod</th><th>Ad</th><th>BSMV</th><th>Tahakkuk</th><th className="w-12 text-right">İşlemler</th>
              </tr>
            </thead>
            <tbody>
              {filteredRows.map((row) => (
                <tr key={row.id} className={selectedKod === row.kod ? 'bg-blue-50/50' : ''} onClick={() => setSelectedKod(row.kod)}>
                  <td>{row.kod}</td><td>{row.ad}</td><td>{row.bsmv}</td><td>{row.tahakkuk}</td>
        ...
        <div className="overflow-auto">
          <div className="p-4 border-b border-slate-200 bg-slate-50">
            ...
            <p className="text-xs text-slate-500 mt-1">Seçili borç tipine ait operasyonel tanımlar</p>
          </div>
          <div className="p-4">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <label><span className="block text-xs font-semibold text-slate-600 mb-1">BSMV</span>{yesNoSelect(form.bsmv, (e) => setForm((f) => ({ ...f, bsmv: e.target.value })))}</label>
          <label><span className="block text-xs font-semibold text-slate-600 mb-1">Tahakkuk</span>{yesNoSelect(form.tahakkuk, (e) => setForm((f) => ({ ...f, tahakkuk: e.target.value })))}</label>
          <label><span className="block text-xs font-semibold text-slate-600 mb-1">Tüm Borç Öde</span>{yesNoSelect(form.tumBorcOde, (e) => setForm((f) => ({ ...f, tumBorcOde: e.target.value })))}</label>
          <label><span className="block text-xs font-semibold text-slate-600 mb-1">Birikime Transfer</span>{yesNoSelect(form.birikimeTransfer, (e) => setForm((f) => ({ ...f, birikimeTransfer: e.target.value })))}</label>
        </div>
      </Modal>
