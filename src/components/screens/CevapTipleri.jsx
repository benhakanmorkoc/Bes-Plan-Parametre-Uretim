...
import { cevapTipleri as seedRows, cevapTipleriDetaylari as seedDetaylar } from '../../data/mockData'

function emptyForm() {
  return { id: null, cevapTipi: '', aciklama: '' }
}

export default function CevapTipleri() {
  const [rows, setRows] = useState(() => seedRows.map((x) => ({ ...x })))
  const [detailMap, setDetailMap] = useState(() => ({ ...seedDetaylar }))
  const [selectedTip, setSelectedTip] = useState(seedRows[0]?.cevapTipi || '')
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => `${r.cevapTipi} ${r.aciklama}`.toLowerCase().includes(q))
  }, [rows, search])

  const selectedRow = rows.find((r) => r.cevapTipi === selectedTip) || filteredRows[0] || null
  const selectedDetails = selectedRow ? detailMap[selectedRow.cevapTipi] || [] : []
  ...
  const saveForm = () => {
    ...
    const existsByType = rows.some((r) => r.cevapTipi === payload.cevapTipi && r.id !== payload.id)
    if (existsByType) return alert('Bu cevap tipi sistemde mevcut.')
    ...
    if (exists) {
      ...
      if (old && old.cevapTipi !== payload.cevapTipi) {
        setDetailMap((prev) => {
          const oldDetails = prev[old.cevapTipi] || []
          const { [old.cevapTipi]: _, ...rest } = prev
          return { ...rest, [payload.cevapTipi]: oldDetails }
        })
      }
    } else {
      setRows((prev) => [...prev, payload])
      setDetailMap((prev) => ({ ...prev, [payload.cevapTipi]: [] }))
    }
    setSelectedTip(payload.cevapTipi)
    setFormOpen(false)
  }
  ...
  const openInspect = (row) => {
    setInfoModal({
      open: true,
      title: 'Cevap Tipi İncele',
      body: <div className="text-sm"><p><strong>Cevap Tipi:</strong> {row.cevapTipi}</p><p><strong>Açıklama:</strong> {row.aciklama}</p></div>,
    })
    setMenuId(null)
  }
  const openVersions = (row) => {
    setInfoModal({
      open: true,
      title: 'Versiyonlar',
      body: <ul className="list-disc pl-5 text-sm"><li>{row.cevapTipi} - Versiyon 1</li><li>{row.cevapTipi} - Versiyon 2</li></ul>,
    })
    setMenuId(null)
  }
  const removeRow = (row) => {
    ...
    setDetailMap((prev) => {
      const { [row.cevapTipi]: _, ...rest } = prev
      return rest
    })
    if (selectedTip === row.cevapTipi) {
      const next = rows.find((r) => r.id !== row.id)
      setSelectedTip(next?.cevapTipi || '')
    }
    setMenuId(null)
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden">
      <ScreenHeader title="Cevap Tipleri" description="Cevap tipi tanımları ve cevaplama davranış detayları" right={<PrimaryButton onClick={openCreate}><Plus className="w-4 h-4" /> Yeni Ekle</PrimaryButton>} />
      <div className="px-6 py-3 bg-slate-50/60 border-b border-slate-100">
        <div className="relative max-w-md">
          <Search className="w-4 h-4 absolute left-3 top-3 text-slate-400" />
          <input className="w-full h-10 pl-9 pr-3 border border-slate-300 rounded-md text-sm" placeholder="Cevap tipi / açıklama ara..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 flex-1 min-h-0">
        <div className="overflow-auto border-r border-slate-200">
          <table className="w-full grid-table">
            <thead><tr><th>Cevap Tipi</th><th>Açıklama</th><th className="w-12 text-right">İşlemler</th></tr></thead>
            <tbody>
              {filteredRows.map((row) => (
                <tr key={row.id} className={selectedTip === row.cevapTipi ? 'bg-blue-50/50' : ''} onClick={() => setSelectedTip(row.cevapTipi)}>
                  <td>{row.cevapTipi}</td><td>{row.aciklama}</td>
        ...
        <div className="overflow-auto">
          <div className="p-4 border-b border-slate-200 bg-slate-50">
            <h3 className="text-sm font-semibold text-slate-800">Alt Detay - {selectedRow ? `${selectedRow.cevapTipi} (${selectedRow.aciklama})` : 'Kayıt seçin'}</h3>
            <p className="text-xs text-slate-500 mt-1">Seçili cevap tipine ait format ve doğrulama parametreleri</p>
          </div>
          <div className="p-4">
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title="Cevap Tipi Kaydı" footer={<><OutlineButton onClick={() => setFormOpen(false)}>Vazgeç</OutlineButton><PrimaryButton onClick={saveForm}>Kaydet</PrimaryButton></>}>
