...
import {
  ygkYillikKademeDonemi as seedRows,
  ygkYillikKademeDonemiDetaylari as seedDetaylar,
} from '../../data/mockData'
...
export default function YgkYillikKademeDonemi() {
  ...
  const openInspect = (row) => {
    setInfoModal({
      open: true,
      title: 'YGK Yıllık Kademe Dönemi İncele',
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden">
      <ScreenHeader title="YGK Yıllık Kademe Dönemi" description="Yıllık kademe dönemi tanımları ve alt parametre detayları" right={<PrimaryButton onClick={openCreate}><Plus className="w-4 h-4" /> Yeni Ekle</PrimaryButton>} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 flex-1 min-h-0">
        ...
        <div className="overflow-auto">
          <div className="p-4 border-b border-slate-200 bg-slate-50">
            ...
            <p className="text-xs text-slate-500 mt-1">Seçili döneme ait kademe uygulama kuralları</p>
          </div>
          <div className="p-4">
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title="YGK Yıllık Kademe Dönemi Kaydı" footer={<><OutlineButton onClick={() => setFormOpen(false)}>Vazgeç</OutlineButton><PrimaryButton onClick={saveForm}>Kaydet</PrimaryButton></>}>
