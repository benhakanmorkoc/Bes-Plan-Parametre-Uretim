...
import { altBranslar as seedRows, altBranslarDetaylari as seedDetaylar } from '../../data/mockData'

function emptyForm() {
  return { id: null, brans: '', kod: '', ad: '', aciklama: '' }
}

export default function AltBranslar() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => `${r.brans} ${r.kod} ${r.ad} ${r.aciklama}`.toLowerCase().includes(q))
  }, [rows, search])
  ...
  const openInspect = (row) => { setInfoModal({ open: true, title: 'Alt Branş İncele', body: <div className="text-sm"><p><strong>Branş:</strong> {row.brans}</p><p><strong>Kod:</strong> {row.kod}</p><p><strong>Açıklama:</strong> {row.aciklama}</p></div> }); setMenuId(null) }
  const openVersions = (row) => { setInfoModal({ open: true, title: 'Versiyonlar', body: <ul className="list-disc pl-5 text-sm"><li>{row.kod} - Versiyon 1</li><li>{row.kod} - Versiyon 2</li></ul> }); setMenuId(null) }
  const removeRow = (row) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden">
      <ScreenHeader title="Alt Branşlar" description="Alt branş tanımları ve alt parametre detayları" right={<PrimaryButton onClick={openCreate}><Plus className="w-4 h-4" /> Yeni Ekle</PrimaryButton>} />
      <div className="px-6 py-3 bg-slate-50/60 border-b border-slate-100">
        <div className="relative max-w-md">
          <Search className="w-4 h-4 absolute left-3 top-3 text-slate-400" />
          <input className="w-full h-10 pl-9 pr-3 border border-slate-300 rounded-md text-sm" placeholder="Branş / kod / açıklama ara..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 flex-1 min-h-0">
        <div className="overflow-auto border-r border-slate-200">
          <table className="w-full grid-table">
            <thead><tr><th>Branş</th><th>Kod</th><th>Açıklama</th><th className="w-12 text-right">İşlemler</th></tr></thead>
            <tbody>
        ...
        <div className="overflow-auto">
          <div className="p-4 border-b border-slate-200 bg-slate-50">
            ...
            <p className="text-xs text-slate-500 mt-1">Seçili alt branşa ait ürün/risk parametreleri</p>
          </div>
          <div className="p-4">
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title="Alt Branş Kaydı" footer={<><OutlineButton onClick={() => setFormOpen(false)}>Vazgeç</OutlineButton><PrimaryButton onClick={saveForm}>Kaydet</PrimaryButton></>}>
