...
import {
  Plus, Search, Link as LinkIcon, ChevronDown, ChevronUp, MoreVertical, Edit2, Trash2, GitBranch, List, History, Eye, ArrowLeft, X, AlertCircle, CheckCircle2,
} from 'lucide-react'
...
export default function KatkiPayiTemplateleriVersiyonRevizyon() {
  ...
  const openVersions = (row) => {
    ...
    const rows = history.length
      ? history.map((h) => {
        const snap = kptData.find((t) => String(t.versiyon) === String(h.versiyon) && t.kpTemplateKodu === kod)
        return { ...h, ...(snap || {}) }
      })
      : kptData.filter((item) => item.kpTemplateKodu === kod)
    setVersionRows([...rows].sort((a, b) => Number(b.versiyon || 0) - Number(a.versiyon || 0)))
    setVersionsOpen(true)
    setMenuRowId(null)
  }

  const openRevisions = (row) => {
    const rows = getRevisions(row.kpTemplateKodu).filter((r) => r.changeType !== 'VERSIYON')
    setRevisionRows([...rows].sort((a, b) => (b.revisionNo || 0) - (a.revisionNo || 0)))
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden relative">
      ...
      <ScreenHeader
        ...
        description="KP template tanımlarının listelendiği, versiyon ve revizyon işlemlerinin yapıldığı ekrandır."
                ...
                <td className="text-center relative">
                  ...
                  {menuRowId === row.id && (
                    <div className="absolute right-8 top-0 mt-1 w-56 bg-white rounded-lg shadow-lg border border-slate-200 z-50 py-1.5 text-left">
                      ...
                      <button type="button" onClick={() => openRevisions(row)} className="w-full flex items-center px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">
                        <History className="w-4 h-4 mr-2 text-violet-500" /> Revizyonlar
                      </button>
                    </div>
                  )}
                </td>
      ...
      >
        <table className="w-full grid-table text-sm">
          <thead>
            <tr>
              <th>Versiyon</th><th>Durum</th><th>Ad</th><th>Tutar</th><th>Azami KP</th><th>Oluşturulma</th><th className="text-center">İşlem</th>
            </tr>
          </thead>
          <tbody>
            {versionRows.map((v) => (
              <tr key={`${v.versiyon}-${v.id}`}>
                ...
                <td>
                  <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${v.aktif !== false ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-600'}`}>
                ...
                <td>{v.azamiKp || '—'}</td>
                <td>{v.olusturulmaTarihi || v.guncellemeTarihi}</td>
                <td className="text-center">
                  <button type="button" className="inline-flex items-center gap-1 text-blue-600 text-xs font-medium" onClick={() => setInspectVersion(v)}>
            ...
            {versionRows.length === 0 && (
              <tr><td colSpan={7} className="text-center py-8 text-slate-400">Versiyon kaydı bulunamadı.</td></tr>
            )}
          </tbody>
        </table>
      </Modal>
      ...
      <Modal
        ...
        title="KP Template Revizyonları"
        size="lg"
        footer={<PrimaryButton onClick={() => setRevisionsOpen(false)}>Kapat</PrimaryButton>}
      >
        <table className="w-full grid-table text-sm">
          <thead>
            <tr>
              <th>Revizyon No</th><th>Versiyon</th><th>Gerekçe</th><th>Alan</th><th>Kullanıcı</th><th>Tarih</th><th className="text-center">İşlem</th>
            </tr>
          </thead>
          <tbody>
            {revisionRows.map((r) => (
              <tr key={r.revisionId}>
                ...
                <td>{r.diff?.length || 0}</td>
                <td>{r.changedBy}</td>
                <td>{r.changedAt}</td>
                <td className="text-center">
                  <button type="button" className="inline-flex items-center gap-1 text-violet-600 text-xs font-medium" onClick={() => setInspectRevision(r)}>
                    <Eye className="w-3.5 h-3.5" /> İncele
                  </button>
                </td>
              </tr>
            ...
            {revisionRows.length === 0 && (
              <tr><td colSpan={7} className="text-center py-8 text-slate-400">Revizyon kaydı bulunamadı.</td></tr>
            )}
          </tbody>
        </table>
      </Modal>
      ...
      >
        {inspectVersion && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            {Object.keys(KPT_FIELD_LABELS).map((field) => (
      ...
      >
        {inspectRevision && (
          <div className="space-y-4">
            ...
            <table className="w-full grid-table text-sm">
              <thead><tr><th>Alan</th><th>Önceki</th><th>Sonraki</th></tr></thead>
              <tbody>
