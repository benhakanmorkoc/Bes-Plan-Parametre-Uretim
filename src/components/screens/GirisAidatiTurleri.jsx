...
import { girisAidatiTurleri as seedRows } from '../../data/mockData'

function emptyForm() {
  return { id: null, kod: '', aciklama: '' }
}

export default function GirisAidatiTurleri() {
  ...
  const openInspect = (row) => {
    setInfoModal({ open: true, title: 'Giriş Aidatı Türü İncele', body: <div className="text-sm"><p><strong>Kod:</strong> {row.kod}</p><p><strong>Açıklama:</strong> {row.aciklama}</p></div> })
    setMenuId(null)
  }

  const openVersions = (row) => {
    setInfoModal({ open: true, title: 'Versiyonlar', body: <ul className="list-disc pl-5 text-sm"><li>{row.kod} - Versiyon 1</li><li>{row.kod} - Versiyon 2</li></ul> })
    setMenuId(null)
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden">
      <ScreenHeader title="Giriş Aidatı Türleri" description="Giriş aidatı tipi tanımları" right={<PrimaryButton onClick={openCreate}><Plus className="w-4 h-4" /> Yeni Ekle</PrimaryButton>} />
      <div className="px-6 py-3 bg-slate-50/60 border-b border-slate-100">
        <div className="relative max-w-md">
          <Search className="w-4 h-4 absolute left-3 top-3 text-slate-400" />
          <input className="w-full h-10 pl-9 pr-3 border border-slate-300 rounded-md text-sm" placeholder="Kod / açıklama ara..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      <div className="flex-1 min-h-0 overflow-auto">
        <table className="w-full grid-table">
          <thead><tr><th>Kod</th><th>Açıklama</th><th className="w-12 text-right">İşlemler</th></tr></thead>
          <tbody>
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title="Giriş Aidatı Türü Kaydı" footer={<><OutlineButton onClick={() => setFormOpen(false)}>Vazgeç</OutlineButton><PrimaryButton onClick={saveForm}>Kaydet</PrimaryButton></>}>
      ...
      <Modal open={infoModal.open} onClose={() => setInfoModal({ open: false, title: '', body: null })} title={infoModal.title} footer={<PrimaryButton onClick={() => setInfoModal({ open: false, title: '', body: null })}>Tamam</PrimaryButton>}>{infoModal.body}</Modal>
    </div>
  )
}
