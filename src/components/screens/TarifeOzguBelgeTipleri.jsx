...
import {
  tarifeOzguBelgeTipleri as seedRows,
  tarifeOzguBelgeTipleriDetaylari as seedDetaylar,
} from '../../data/mockData'
...
export default function TarifeOzguBelgeTipleri() {
  ...
  const openInspect = (row) => {
    setInfoModal({
      open: true,
      title: 'Tarife Özgü Belge Tipi İncele',
      body: <div className="text-sm"><p><strong>Kod:</strong> {row.kod}</p><p><strong>Açıklama:</strong> {row.aciklama}</p></div>,
    })
    setMenuId(null)
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden">
      <ScreenHeader
        title="Tarife Özgü Belge Tipleri"
        description="Belge tipi tanımları ve belge akışına ait alt detaylar"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 flex-1 min-h-0">
            ...
            <tbody>
              {filteredRows.map((row) => (
                <tr key={row.id} className={selectedKod === row.kod ? 'bg-blue-50/50' : ''} onClick={() => setSelectedKod(row.kod)}>
                  <td>{row.kod}</td><td>{row.aciklama}</td>
        ...
        <div className="overflow-auto">
          <div className="p-4 border-b border-slate-200 bg-slate-50">
            ...
            <p className="text-xs text-slate-500 mt-1">Seçili belge tipine ait operasyonel kurallar</p>
          </div>
          <div className="p-4">
      ...
      <Modal
        ...
        title="Tarife Özgü Belge Tipi Kaydı"
