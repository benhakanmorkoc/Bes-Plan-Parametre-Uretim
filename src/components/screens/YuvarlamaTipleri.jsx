...
import {
  yuvarlamaTipleri as seedRows,
  yuvarlamaTipleriDetaylari as seedDetaylar,
} from '../../data/mockData'

function emptyForm() {
  return { id: null, ad: '', kod: '', aciklama: '' }
}

export default function YuvarlamaTipleri() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => `${r.ad} ${r.kod} ${r.aciklama}`.toLowerCase().includes(q))
  }, [rows, search])
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden">
      <ScreenHeader
        ...
        description="Yuvarlama tipi tanımları ve hesaplama detayları"
      ...
      <div className="px-6 py-3 bg-slate-50/60 border-b border-slate-100">
        <div className="relative max-w-md">
          ...
          <input className="w-full h-10 pl-9 pr-3 border border-slate-300 rounded-md text-sm" placeholder="Ad / kod / açıklama ara..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 flex-1 min-h-0">
        <div className="overflow-auto border-r border-slate-200">
          <table className="w-full grid-table">
            <thead><tr><th>Ad</th><th>Kod</th><th>Açıklama</th><th className="w-12 text-right">İşlemler</th></tr></thead>
            <tbody>
              {filteredRows.map((row) => (
                <tr key={row.id} className={selectedKod === row.kod ? 'bg-blue-50/50' : ''} onClick={() => setSelectedKod(row.kod)}>
                  <td>{row.ad}</td><td>{row.kod}</td><td>{row.aciklama}</td>
        ...
        <div className="overflow-auto">
          <div className="p-4 border-b border-slate-200 bg-slate-50">
            ...
            <p className="text-xs text-slate-500 mt-1">Seçili yuvarlama tipine ait hesaplama parametreleri</p>
          </div>
          <div className="p-4">
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title="Yuvarlama Tipi Kaydı" footer={<><OutlineButton onClick={() => setFormOpen(false)}>Vazgeç</OutlineButton><PrimaryButton onClick={saveForm}>Kaydet</PrimaryButton></>}>
