...
import { odemeDonemiTurleri as seedRows } from '../../data/mockData'

function emptyForm() {
  return { id: null, kod: '', aciklama: '' }
}

export default function OdemeDonemiTurleri() {
  ...
  const openInspect = (row) => {
    setInfoModal({ open: true, title: 'Ödeme Dönemi Türü İncele', body: <div className="text-sm"><p><strong>Kod:</strong> {row.kod}</p><p><strong>Açıklama:</strong> {row.aciklama}</p></div> })
    setMenuId(null)
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-full overflow-hidden">
      <ScreenHeader title="Ödeme Dönemi Türleri" description="Ödeme dönemi tanımları" right={<PrimaryButton onClick={openCreate}><Plus className="w-4 h-4" /> Yeni Ekle</PrimaryButton>} />
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title="Ödeme Dönemi Türü Kaydı" footer={<><OutlineButton onClick={() => setFormOpen(false)}>Vazgeç</OutlineButton><PrimaryButton onClick={saveForm}>Kaydet</PrimaryButton></>}>
